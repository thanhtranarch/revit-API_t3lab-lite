...
XAML_FILE     = os.path.join(LIB_DIR, 'GUI', 'Tools', 'ManaLoca.xaml')
...
class LocationManagerHandler(IExternalEventHandler):
    ...
    def _report_error(self, msg):
        ...
        ExternalEvent callback are invisible to the user (print() only goes
        to a console they likely never opened), so the window just looks
...
class LocationManagerWindow(forms.WPFWindow):
    def __init__(self, xaml_file_path):
        ...
        try:
            ...
            msg = str(ex) + "\n\n"
            if hasattr(ex, "inner"):
                msg += "Inner: " + str(ex.inner)
            elif hasattr(ex, "__cause__") and ex.__cause__:
                msg += "Cause: " + str(ex.__cause__)
            elif hasattr(ex, "data"):
                msg += "Data: " + str(ex.data)
            from System.Windows import MessageBox, MessageBoxButton, MessageBoxImage
            MessageBox.Show(msg, "XAML Load Error", MessageBoxButton.OK, MessageBoxImage.Error)
            raise
