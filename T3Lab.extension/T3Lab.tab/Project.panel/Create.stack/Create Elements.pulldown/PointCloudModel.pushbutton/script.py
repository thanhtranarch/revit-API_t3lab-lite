...
class PointCloudModelWindow(forms.WPFWindow):
    ...
    def _go_to_step(self, step):
        ...
        circles   = [self.step_circle_1,   self.step_circle_2,   self.step_circle_3]
        labels    = [self.step_label_1,    self.step_label_2,    self.step_label_3]
        underlines = [self.step_underline_1, self.step_underline_2, self.step_underline_3]
        nums      = [self.step_num_1,      self.step_num_2,      self.step_num_3]
        ...
        self.btn_generate.Visibility = (Visibility.Visible
                                        if step == 2 else Visibility.Collapsed)
    ...
    def btn_next_clicked(self, sender, e):
        ...
        if self.rb_custom_region.IsChecked and self._custom_min_pt is None:
            TaskDialog.Show(
                "Point Cloud to Model",
                "You selected 'Custom Region' but no region was picked.\n"
                "Click 'Pick Region Corners' first, or switch to 'Full Cloud Extent'.")
            return
        self._go_to_step(1)
        self.status_text.Text = u"Configure detection settings, then click Analyze."
    ...
    def _read_settings(self):
        def fval(ctrl, default):
            try:
                return float(ctrl.Text)
            except Exception:
                return default

        return {
            'detect_wall':    bool(self.chk_detect_wall.IsChecked),
            'snap_tol':       fval(self.txt_snap_tol,       1.0),
            'wall_min_len':   fval(self.txt_wall_min_len,   500.0),
            'detect_floor':   bool(self.chk_detect_floor.IsChecked),
            'floor_zbin':     fval(self.txt_floor_zbin,     50.0),
            'floor_min_area': fval(self.txt_floor_min_area, 1.0),
            'detect_ceiling': bool(self.chk_detect_ceiling.IsChecked),
            'ceil_zbin':      fval(self.txt_ceil_zbin,      50.0),
            'detect_door':    bool(self.chk_detect_door.IsChecked),
            'door_min_w':     fval(self.txt_door_min_w,     600.0),
            'door_min_h':     fval(self.txt_door_min_h,     1800.0),
            'detect_window':  bool(self.chk_detect_window.IsChecked),
            'win_min_w':      fval(self.txt_win_min_w,      400.0),
            'win_min_h':      fval(self.txt_win_min_h,      400.0),
            'detect_column':  bool(self.chk_detect_column.IsChecked),
            'col_min':        fval(self.txt_col_min,        100.0),
            'col_max':        fval(self.txt_col_max,        600.0),
            'detect_stair':   bool(self.chk_detect_stair.IsChecked),
            'stair_riser':    fval(self.txt_stair_riser,    165.0),
            'detect_roof':    bool(self.chk_detect_roof.IsChecked),
            'roof_slope':     fval(self.txt_roof_slope,     5.0),
        }
