...
class EditRecordWindow(forms.WPFWindow):
    def __init__(self,
        ...
        try:
            # Get absolute path to XAML file
            xaml_file_path = op.join(op.dirname(__file__), 'EditRecord.xaml')
            ...
            logger.error("Error initializing EditRecordWindow: {}".format(ex))
            raise
...
class KeynoteManagerWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name, reset_config=False):
        try:
            # Get absolute path to XAML file
            xaml_file_path = op.join(op.dirname(__file__), xaml_file_name)
            forms.WPFWindow.__init__(self, xaml_file_path)
            ...
            logger.error("Error initializing KeynoteManagerWindow: {}".format(ex))
            raise
    ...
    def rekey_keynote_refs(self, from_key, to_key):
        with revit.Transaction("Rekey Keynote {}".format(from_key)):
            used_elements = self.get_used_keynote_elements()
            if used_elements is not None:
                for kid in used_elements.get(from_key, []):
    ...
    def show_keynote(self, sender, args):
        if self.selected_keynote:
            self.Close()
            used_elements = self.get_used_keynote_elements()
            kids = used_elements.get(self.selected_keynote.key, []) if used_elements is not None else []
            for kid in kids:
                source = viewname = ''
                kel = revit.doc.GetElement(kid)
