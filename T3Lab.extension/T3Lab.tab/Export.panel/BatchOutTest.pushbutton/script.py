...
class ExportManagerWindow(forms.WPFWindow):
    ...
    def __init__(self):
        try:
            ...
            extension_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
            xaml_file_path = os.path.join(extension_dir, 'lib', 'GUI', 'Tools', 'ExportManagerTest.xaml')
    ...
    def update_navigation_buttons(self):
        ...
        # Setup brushes for bottom navigation bar
        from System.Windows.Media import BrushConverter
        from System.Windows import Visibility
        bc = BrushConverter()
        active_bg = bc.ConvertFromString("#F0F8FF")
        active_fg = bc.ConvertFromString("#005B82")
        inactive_bg = bc.ConvertFromString("Transparent")
        inactive_fg = bc.ConvertFromString("#7F8C8D")
        
        # Helper to set colors
        def set_tab_style(tab_index):
            if hasattr(self, 'nav_border_selection'):
                self.nav_border_selection.Background = active_bg if tab_index == 0 else inactive_bg
                self.nav_icon_selection.Foreground = active_fg if tab_index == 0 else inactive_fg
                self.nav_text_selection.Foreground = active_fg if tab_index == 0 else inactive_fg
            if hasattr(self, 'nav_border_format'):
                self.nav_border_format.Background = active_bg if tab_index == 1 else inactive_bg
                self.nav_icon_format.Foreground = active_fg if tab_index == 1 else inactive_fg
                self.nav_text_format.Foreground = active_fg if tab_index == 1 else inactive_fg
            if hasattr(self, 'nav_border_queue'):
                self.nav_border_queue.Background = active_bg if tab_index == 2 else inactive_bg
                self.nav_icon_queue.Foreground = active_fg if tab_index == 2 else inactive_fg
                self.nav_text_queue.Foreground = active_fg if tab_index == 2 else inactive_fg
            if hasattr(self, 'nav_border_settings'):
                self.nav_border_settings.Background = active_bg if tab_index == 3 else inactive_bg
                self.nav_icon_settings.Foreground = active_fg if tab_index == 3 else inactive_fg
                self.nav_text_settings.Foreground = active_fg if tab_index == 3 else inactive_fg

        set_tab_style(current_tab_index)

        if current_tab_index == 0:  # Selection tab
            self.back_button.Visibility = Visibility.Collapsed
            self.next_button_text.Text = "Next"
            self.next_button_icon.Visibility = Visibility.Visible
            ...
            self.next_button_icon.Visibility = Visibility.Collapsed
