# -*- coding: utf-8 -*-
"""Align Positions Dialog — snaps element positions to grid/wall/column references."""
                ...
                try:
                    ...
                    if res["new_curve"]:
                        if isinstance(elem, Wall):
                            elem.Location.Curve = res["new_curve"]
                        elif isinstance(elem, Grid):
                            import Autodesk.Revit.DB as DB
                            try:
                                elem.SetCurveInView(DB.DatumExtentType.Model, self.doc.ActiveView, res["new_curve"])
                            except:
...
class AlignPositionsWindow(forms.WPFWindow):
    def __init__(self, uidoc_ref, doc_ref, parent_window=None):
    ...
    # ── Window chrome handlers (unused when nested but kept to prevent XAML errors) ──
    ...
    def _snap_feet(self):
        ...
        if is_metric:
            if self.rb_snap1.IsChecked:
                return 5.0 / MM_PER_FOOT
            if self.rb_snap2.IsChecked:
                return 10.0 / MM_PER_FOOT
            if getattr(self, 'rb_snap3', None) and self.rb_snap3.IsChecked:
                return 20.0 / MM_PER_FOOT
            return 5.0 / MM_PER_FOOT
        # Imperial: 1/8 inch
        return (1.0 / 8.0) / 12.0

    def _set_reference(self, element):
        self._aligner = PositionAligner(self.doc, element)
    ...
    def _auto_find_elements(self):
        if not self._aligner:
            return
        walls = FilteredElementCollector(self.doc, self.doc.ActiveView.Id).OfClass(Wall).ToElements()
        grids = FilteredElementCollector(self.doc, self.doc.ActiveView.Id).OfClass(Grid).ToElements()
    ...
    def _analyze(self, elements):
        ...
        for res in self._results:
            ...
            # Start and End points for curve-based elements
            if isinstance(elem, Wall) and hasattr(elem.Location, "Curve"):
                c = elem.Location.Curve
                row["StartPt"] = "({:.1f}, {:.1f})".format(feet_to_mm(c.GetEndPoint(0).X), feet_to_mm(c.GetEndPoint(0).Y))
                row["EndPt"] = "({:.1f}, {:.1f})".format(feet_to_mm(c.GetEndPoint(1).X), feet_to_mm(c.GetEndPoint(1).Y))
            elif isinstance(elem, Grid):
                c = elem.Curve
                row["StartPt"] = "({:.1f}, {:.1f})".format(feet_to_mm(c.GetEndPoint(0).X), feet_to_mm(c.GetEndPoint(0).Y))
                row["EndPt"] = "({:.1f}, {:.1f})".format(feet_to_mm(c.GetEndPoint(1).X), feet_to_mm(c.GetEndPoint(1).Y))
    ...
    def apply_clicked(self, sender, e):
        ...
        # Filter results based on CheckBox selection in DataGrid
        active_results = []
        for i, row in enumerate(self._dt.Rows):
            if row["Apply"]:
                active_results.append(self._results[i])
                
        if not active_results:
            forms.alert("No elements selected for correction.")
            return

        moved = self._aligner.apply_corrections(active_results)
