# -*- coding: utf-8 -*-
"""
Line Style Manager (Standalone)
Manage Line Styles with beautiful UI - Sheet Manager Style
...
from Autodesk.Revit.DB import (FilteredElementCollector, Transaction, TransactionGroup,
                                ElementId, BuiltInCategory, GraphicsStyleType,
                                CurveElement, BuiltInParameter)
...
# ============================================================================
# LINE STYLE ITEM (Data Model)
# ============================================================================

class LineStyleItem(INotifyPropertyChanged):
    """Wrapper class for Line Style (Category) with WPF binding support"""
    ...
    def __init__(self, category):
        ...
        self._id = category.Id.IntegerValue
        ...
        self._color_rgb = self._get_color_rgb()
    ...
    def _get_weight(self):
        ...
        try:
            ...
            return str(weight) if weight else "1"
        except:
            return "N/A"
    
    def _get_color(self):
        """Get line color as Color object"""
        try:
    ...
    def _get_pattern(self):
        ...
        try:
            ...
            if pattern_id and pattern_id != ElementId.InvalidElementId:
    ...
    def color_brush(self):
        return SolidColorBrush(self._color)
...
def calculate_linestyle_usage(doc, line_style_items):
    """Calculate usage for line styles by counting CurveElements"""
    ...
    for item in line_style_items:
        if item.category:
            category_lookup[item.category.Id.IntegerValue] = item
    ...
    try:
        ...
        for curve in collector:
            try:
                # Get GraphicsStyle from parameter
                ...
                if hasattr(style_elem, 'GraphicsStyleCategory'):
                    ...
                    if style_cat:
                        cat_id = style_cat.Id.IntegerValue
    ...
    for item in line_style_items:
        if item.category:
            cat_id = item.category.Id.IntegerValue
            if cat_id in usage_dict:
                item.usage_count = usage_dict[cat_id]
...
class BatchRenameDialog(Window):
    ...
    def __init__(self, items, parent_window):
        ...
        # Tab 6: Style Info
        self.add_weight_check = None
        self.add_color_check = None
        self.add_pattern_check = None
    ...
    def _build_ui(self):
        """Build dialog UI"""
        self.Title = "Batch Rename Line Styles"
        self.Width = 600
        self.Height = 550
        ...
        info = TextBlock()
        info.Text = "{} style(s) selected for renaming".format(len(self.items))
        info.FontSize = 12
    ...
    def _create_tabs(self):
        ...
        tab6 = TabItem()
        tab6.Header = "Style Info"
        tab6.Content = self._create_style_info_tab()
    ...
    def _create_style_info_tab(self):
        """Create Style Info tab - add style properties to name"""
        panel = StackPanel()
        panel.Margin = Thickness(10)
        
        title = TextBlock()
        title.Text = "Add style properties to name:"
        title.FontWeight = FontWeights.SemiBold
        title.Margin = Thickness(0, 0, 0, 10)
        panel.Children.Add(title)
        
        self.add_weight_check = CheckBox()
        self.add_weight_check.Content = "Add Weight (1, 2, 3...)"
        self.add_weight_check.Margin = Thickness(0, 5, 0, 5)
        self.add_weight_check.Checked += lambda s, e: self._update_preview()
        self.add_weight_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_weight_check)
        
        self.add_color_check = CheckBox()
        self.add_color_check.Content = "Add Color (R-G-B)"
        self.add_color_check.Margin = Thickness(0, 0, 0, 5)
        ...
        self.add_pattern_check = CheckBox()
        self.add_pattern_check.Content = "Add Pattern (Solid, Dash...)"
        self.add_pattern_check.Margin = Thickness(0, 0, 0, 10)
        self.add_pattern_check.Checked += lambda s, e: self._update_preview()
        self.add_pattern_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_pattern_check)
        
        self.remove_current_name_check = CheckBox()
        self.remove_current_name_check.Content = "Remove current name (use only style info)"
        ...
        example = TextBlock()
        example.Text = "Examples:\n  'MyLine' + (W3) -> 'MyLine_(W3)'\n  'Style' + (0-0-0)(Solid) -> 'Style_(0-0-0)(Solid)'"
        example.FontSize = 10
    ...
    def _get_color_short(self, item):
        """Get color as R-G-B format"""
        try:
    ...
    def _apply_rules_to_name(self, name, index):
        ...
        # Tab 5: Style Info
            ...
            if item:
                ...
                if self.add_weight_check.IsChecked:
                    info_parts.append("W{}".format(item.weight))
                
                if self.add_color_check.IsChecked:
                    info_parts.append(self._get_color_short(item))
                
                if self.add_pattern_check.IsChecked:
                    info_parts.append(item.pattern)
    ...
    def _on_apply(self, sender, args):
        ...
        # Line styles need special rename method (create-copy-transfer-delete)
        success_count = 0
        error_count = 0
        
        for item, new_name in changes:
            if self.parent_window._rename_line_style(item, new_name, skip_prompts=True):
                success_count += 1
            else:
                error_count += 1
...
class LineStyleManagerWindow(Window):
    """Line Style Manager with Sheet Manager style UI"""
    ...
    def _create_header(self):
        ...
        title = TextBlock()
        title.Text = "Line Style Manager v1.0"
        title.FontSize = 17
        title.FontWeight = FontWeights.Bold
        panel.Children.Add(title)
    ...
    def _create_left_panel(self):
        ...
        for item_text in ["All Styles", "System Only", "Custom Only"]:
            item = ComboBoxItem()
    ...
    def _create_datagrid(self):
        ...
        col_weight.Width = DataGridLength(60)
        ...
        color_binding = Binding("color_brush")
        border_factory.SetBinding(Border.BackgroundProperty, color_binding)
        ...
        text_binding = Binding("color_rgb")
        text_factory.SetBinding(TextBlock.TextProperty, text_binding)
        ...
        col_pattern.Width = DataGridLength(120)
        grid.Columns.Add(col_pattern)
        
        col_usage = DataGridTextColumn()
        col_usage.Header = "Line Count"
    ...
    def _create_tips(self):
        ...
        tips = TextBlock()
        tips.Text = "Click row to select | System styles (< >) cannot be renamed/deleted"
        tips.FontSize = 10
    ...
    def _load_data(self):
        ...
        try:
            ...
            if not lines_category:
                MessageBox.Show("Could not find Lines category!", "Error",
            ...
            if lines_category.SubCategories:
                for subcat in lines_category.SubCategories:
                    try:
                        item = LineStyleItem(subcat)
                        self.all_items.append(item)
                    except Exception as ex:
                        print("Error loading line style: {}".format(str(ex)))
            ...
            MessageBox.Show("Failed to load line styles:\n\n{}".format(str(ex)),
    ...
    def _update_stats(self):
        ...
        if self.txt_system:
            system = sum(1 for item in self.all_items if item.is_system)
            self.txt_system.Text = str(system)
        
        if self.txt_custom:
            custom = sum(1 for item in self.all_items if not item.is_system)
    ...
    # ========================================================================
    # LINE STYLE RENAME METHOD (Create-Copy-Transfer-Delete)
    # ========================================================================
    
    def _rename_line_style(self, item, new_name=None, skip_prompts=False):
        """Rename line style using create-copy-transfer-delete method"""
        old_style = item.category
        
        # Check if system style
        if item.is_system:
            if not skip_prompts:
                MessageBox.Show("Cannot rename system line style '{}'!".format(item.name),
                              "Error", MessageBoxButton.OK, MessageBoxImage.Error)
            return False
        ...
        if lines_category.SubCategories:
            for subcat in lines_category.SubCategories:
                if subcat.Id != old_style.Id:
                    try:
                        if subcat.Name == new_name:
                            if not skip_prompts:
                                MessageBox.Show("A line style '{}' already exists!".format(new_name),
                                              "Error", MessageBoxButton.OK, MessageBoxImage.Error)
                            return False
                    except:
                        continue
        ...
        try:
            ...
            try:
                ...
                try:
                    old_weight = old_style.GetLineWeight(GraphicsStyleType.Projection)
                    if old_weight:
                        new_subcategory.SetLineWeight(old_weight, GraphicsStyleType.Projection)
                except:
                    pass
                
                try:
                    old_pattern_id = old_style.GetLinePatternId(GraphicsStyleType.Projection)
                    if old_pattern_id and old_pattern_id != ElementId.InvalidElementId:
                        new_subcategory.SetLinePatternId(old_pattern_id, GraphicsStyleType.Projection)
                except:
                    pass
                ...
                t1.RollBack()
                tg.RollBack()
                print("Error creating new style: {}".format(str(ex)))
                return False
            
            # Step 2: Transfer existing lines to new style
            t2 = Transaction(doc, "Transfer Lines")
            t2.Start()
            
            try:
                # Get old and new GraphicsStyle
                old_graphics_style = old_style.GetGraphicsStyle(GraphicsStyleType.Projection)
                new_graphics_style = new_subcategory.GetGraphicsStyle(GraphicsStyleType.Projection)
                
                if old_graphics_style and new_graphics_style:
                    # Find all curves using old style
                    collector = FilteredElementCollector(doc).OfClass(CurveElement)
                    ...
                    for curve in collector:
                        try:
                            style_param = curve.get_Parameter(BuiltInParameter.BUILDING_CURVE_GSTYLE)
                            if style_param:
                                current_style_id = style_param.AsElementId()
                                if current_style_id == old_graphics_style.Id:
                                    style_param.Set(new_graphics_style.Id)
                        except:
                            pass
                ...
                t2.RollBack()
                # Continue anyway - lines may not transfer but rename can still work
                print("Warning: Could not transfer lines: {}".format(str(ex)))
            
            # Step 3: Delete old style
            t3 = Transaction(doc, "Delete Old Style")
            t3.Start()
            ...
            try:
                ...
                t3.RollBack()
                print("Warning: Could not delete old style: {}".format(str(ex)))
            
            tg.Assimilate()
            return True
            
        except Exception as ex:
            print("Error renaming line style: {}".format(str(ex)))
            if not skip_prompts:
    ...
    def _on_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select one line style to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        
        if len(selected) > 1:
            MessageBox.Show("Please select only one line style to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        
        item = selected[0]
        
        if self._rename_line_style(item):
            self._load_data()
    ...
    def _on_batch_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one line style to batch rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        if not renamable:
            MessageBox.Show("Cannot rename system line styles!\nPlease select custom styles.",
                          "Error", MessageBoxButton.OK, MessageBoxImage.Error)
            return
        
        if len(renamable) < len(selected):
            MessageBox.Show("{} system style(s) will be skipped.\n{} custom style(s) will be renamed.".format(
    ...
    def _on_delete(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one line style to delete!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        # Check for styles in use
        in_use = [item for item in deletable if item.usage_count > 0]
        
        if in_use:
            msg = "WARNING: {} style(s) are in use:\n\n".format(len(in_use))
            for item in in_use[:5]:
                msg += "  - '{}': {} lines\n".format(item.name, item.usage_count)
            if len(in_use) > 5:
                msg += "  ... and {} more\n".format(len(in_use) - 5)
            msg += "\nDeleting will affect existing lines. Continue?"
            
            result = MessageBox.Show(msg, "Styles In Use",
                                    MessageBoxButton.YesNo, MessageBoxImage.Warning)
            if result != MessageBoxResult.Yes:
                return
        else:
            result = MessageBox.Show(
                "Delete {} line style(s)?".format(len(deletable)),
                "Confirm Delete",
...
if __name__ == '__main__':
    try:
        ...
        MessageBox.Show(
            "Error starting Line Style Manager:\n\n{}".format(str(e)),
            "Error",
            MessageBoxButton.OK,
