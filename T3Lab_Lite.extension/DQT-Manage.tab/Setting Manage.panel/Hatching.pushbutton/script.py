# -*- coding: utf-8 -*-
"""
Fill Pattern Manager (Standalone)
Manage Fill Patterns (Hatching) with beautiful UI - Sheet Manager Style
...
from Autodesk.Revit.DB import (FilteredElementCollector, FillPatternElement,
                                FillPatternTarget, Transaction, ElementId)
...
# ============================================================================
# FILL PATTERN ITEM (Data Model)
# ============================================================================

class FillPatternItem(INotifyPropertyChanged):
    """Wrapper class for FillPatternElement with WPF binding support"""
    ...
    def __init__(self, element):
        ...
        self._pattern_type = self._get_pattern_type()
        self._pattern_settings = self._get_pattern_settings()
        self._grid_count = self._get_grid_count()
        self._is_solid = self._check_is_solid()
        self._is_system = self._check_is_system()
    
    def _get_pattern_type(self):
        try:
    ...
    def _check_is_solid(self):
        try:
            fill_pattern = self._element.GetFillPattern()
            if fill_pattern:
                return fill_pattern.IsSolidFill
            return False
        except:
            return False
    
    def _check_is_system(self):
        """Check if this is a system/built-in pattern"""
        try:
            name = self._name
            if name and (name.startswith("<") or "Solid fill" in name):
                return True
            return False
        except:
            return False
    
    def _get_pattern_settings(self):
        try:
            fill_pattern = self._element.GetFillPattern()
            if not fill_pattern:
                return "N/A"
            ...
            if grid_count == 1:
                ...
                grid1 = grids[0]
                grid2 = grids[1]
                angle1_deg = math.degrees(grid1.Angle)
                angle2_deg = math.degrees(grid2.Angle)
            ...
            try:
                first_grid = grids[0]
                offset_mm = first_grid.Offset * 304.8
                if offset_mm > 0:
...
class BatchRenameDialog(Window):
    ...
    def __init__(self, items, parent_window):
        ...
        self.add_settings_check = None
        self.add_grids_check = None
    ...
    def _build_ui(self):
        """Build dialog UI"""
        self.Title = "Batch Rename Fill Patterns"
        self.Width = 600
        self.Height = 550
    ...
    def _create_pattern_info_tab(self):
        ...
        # Checkboxes for what to add
        self.add_type_check = CheckBox()
        self.add_type_check.Content = "Add Type (Drafting/Model)"
        self.add_type_check.Margin = Thickness(0, 5, 0, 5)
        ...
        self.add_settings_check = CheckBox()
        self.add_settings_check.Content = "Add Settings (Parallel Lines, Crosshatch, Solid Fill...)"
        self.add_settings_check.Margin = Thickness(0, 0, 0, 5)
        self.add_settings_check.Checked += lambda s, e: self._update_preview()
        self.add_settings_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_settings_check)
        
        self.add_grids_check = CheckBox()
        self.add_grids_check.Content = "Add Grid Count"
        self.add_grids_check.Margin = Thickness(0, 0, 0, 10)
        self.add_grids_check.Checked += lambda s, e: self._update_preview()
        self.add_grids_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_grids_check)
        ...
        self.info_position_combo.SelectedIndex = 1  # Default: Suffix
        ...
        for option in ["None", "[Square]", "(Parentheses)", "{Curly}", "<Angle>"]:
            item = ComboBoxItem()
            item.Content = option
            self.info_bracket_combo.Items.Add(item)
        
        self.info_bracket_combo.SelectedIndex = 1  # Default: [Square]
        ...
        # Example
        example = TextBlock()
        example.Text = "Examples:\n  'Diagonal' + [Crosshatch] -> 'Diagonal [Crosshatch]'\n  'Brick' + [Model][Parallel Lines] -> 'Brick [Model][Parallel Lines]'"
        example.FontSize = 10
    ...
    def _apply_rules_to_name(self, name, index):
        ...
        # Tab 3: Case Change
            ...
            if case_option == 1:  # UPPERCASE
                new_name = new_name.upper()
            elif case_option == 2:  # lowercase
                new_name = new_name.lower()
            elif case_option == 3:  # Title Case
                new_name = new_name.title()
            elif case_option == 4:  # Sentence case
        ...
        # Tab 5: Pattern Info
            ...
            # Get the item to access its properties
            item = self.items[index] if index < len(self.items) else None
            if item:
                info_parts = []
                
                # Collect pattern info based on checkboxes
                if self.add_type_check.IsChecked:
                    info_parts.append(item.pattern_type)
                
                if self.add_settings_check.IsChecked:
                    # Get clean settings (remove spacing info for cleaner name)
                    settings = item.pattern_settings
                    if " - " in settings:
                        settings = settings.split(" - ")[0]  # Remove spacing info
                    info_parts.append(settings)
                
                if self.add_grids_check.IsChecked:
                    info_parts.append("{}grids".format(item.grid_count))
                ...
                if info_parts:
                    ...
                    # Get separator
                    separator = self.info_separator_box.Text if self.info_separator_box.Text else " "
                    
                    # Check if remove current name
    ...
    def _on_apply(self, sender, args):
        ...
        t = Transaction(doc, "Batch Rename Fill Patterns")
        t.Start()
...
class FillPatternManagerWindow(Window):
    """Fill Pattern Manager with Sheet Manager style UI"""
    ...
    def _build_ui(self):
        """Build the main UI"""
        self.Title = "Fill Pattern Manager"
        self.Width = 1000
        self.Height = 700
        self.WindowStartupLocation = System.Windows.WindowStartupLocation.CenterScreen
        self.Background = SolidColorBrush(Config.hex_to_color(Config.BACKGROUND_COLOR))
    ...
    def _create_header(self):
        ...
        title = TextBlock()
        title.Text = "Fill Pattern Manager v1.0"
        title.FontSize = 17
        title.FontWeight = FontWeights.Bold
        panel.Children.Add(title)
    ...
    def _create_summary_cards(self):
        ...
        card3, self.txt_drafting = self._create_card("DRAFTING", "0", 2, Config.SUCCESS_COLOR)
        Grid.SetColumn(card3, 2)
        grid.Children.Add(card3)
        
        card4, self.txt_model = self._create_card("MODEL", "0", 3, "#666666")
    ...
    def _create_left_panel(self):
        ...
        for item_text in ["All Patterns", "Drafting Only", "Model Only", "Solid Fill", "Custom Patterns"]:
            item = ComboBoxItem()
    ...
    def _create_datagrid(self):
        ...
        col_settings.Header = "Pattern Settings"
        col_settings.Binding = Binding("pattern_settings")
        col_settings.Width = DataGridLength(250)
        grid.Columns.Add(col_settings)
        
        col_grids = DataGridTextColumn()
        col_grids.Header = "Grids"
        col_grids.Binding = Binding("grid_count")
        col_grids.Width = DataGridLength(60)
        grid.Columns.Add(col_grids)
    ...
    def _create_action_buttons(self):
        ...
        btn_dup = self._create_button("Duplicate", primary)
        btn_dup.Click += self._on_duplicate
    ...
    def _create_tips(self):
        ...
        tips = TextBlock()
        tips.Text = "Click row to select | Shift+Click for range | Space Bar to toggle checkbox"
        tips.FontSize = 10
    ...
    def _apply_filters(self):
        ...
        for item in self.all_items:
            ...
            if filter_index == 1:
                if item.pattern_type != "Drafting":
                    continue
            elif filter_index == 2:
                if item.pattern_type != "Model":
                    continue
            elif filter_index == 3:
                if not item.is_solid:
                    continue
            elif filter_index == 4:
                if item.is_solid:
                    continue
    ...
    def _update_stats(self):
        ...
        if self.txt_drafting:
            drafting = sum(1 for item in self.all_items if item.pattern_type == "Drafting")
            self.txt_drafting.Text = str(drafting)
        
        if self.txt_model:
            model = sum(1 for item in self.all_items if item.pattern_type == "Model")
            self.txt_model.Text = str(model)
    ...
    def _on_select_unused(self, sender, args):
        self.data_grid.UnselectAll()
        for item in self.filtered_items:
            item.is_selected = not item.is_system and not item.is_solid
        self.data_grid.Items.Refresh()
        self._update_stats()
        MessageBox.Show("Selected non-system custom patterns.\nNote: This doesn't check actual usage in model.",
    ...
    def _on_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select one fill pattern to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        
        if len(selected) > 1:
            MessageBox.Show("Please select only one fill pattern to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        if item.is_system:
            MessageBox.Show("Cannot rename system fill patterns!",
        ...
        for other in self.all_items:
            if other.id != item.id and other.name == new_name:
                MessageBox.Show("A fill pattern with name '{}' already exists!".format(new_name),
                              "Error", MessageBoxButton.OK, MessageBoxImage.Error)
                return
        ...
        try:
            ...
            MessageBox.Show("Fill pattern renamed successfully!",
    ...
    def _on_batch_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one fill pattern to batch rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
    ...
    def _on_duplicate(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one fill pattern to duplicate!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        try:
            ...
            for item in selected:
                try:
                    ...
                    new_id = item.element.Duplicate(new_name)
            ...
            t.RollBack()
            MessageBox.Show("Error duplicating:\n\n{}".format(str(ex)),
                          "Error", MessageBoxButton.OK, MessageBoxImage.Error)
    ...
    def _on_delete(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one fill pattern to delete!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        if not deletable:
            MessageBox.Show("Cannot delete system fill patterns!",
                          "Error", MessageBoxButton.OK, MessageBoxImage.Error)
            return
        
        result = MessageBox.Show(
            "Delete {} fill pattern(s)?\n\nNote: System patterns will be skipped.".format(len(deletable)),
            "Confirm Delete",
...
if __name__ == '__main__':
    try:
        window = FillPatternManagerWindow()
        window.ShowDialog()
