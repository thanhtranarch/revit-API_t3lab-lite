...
class BulkFamilyExportWindow(forms.WPFWindow):
    ...
    def _scan_entire_cad(self, import_inst):
        ...
        def collect_curves(geo_elem):
            """Collect all valid curves recursively, including Solid edges."""
            curves = []
                    ...
                    for curve in curves:
                        try:
                            ...
                            if isinstance(new_c, Line):
                                # Exact: rebuild from endpoints forced to Z=0
                            ...
                            elif isinstance(new_c, Arc):
                                # Use Evaluate() at the PARAMETRIC midpoint
                                # to get a point that is guaranteed ON the arc,
                                # then project everything to Z=0.
                                written = False
                                try:
                                    t0 = new_c.GetEndParameter(0)
                                    t1 = new_c.GetEndParameter(1)
                                    pm = new_c.Evaluate(
                                        (t0 + t1) / 2.0, False)   # point ON arc
                                    p0f = XYZ(new_c.GetEndPoint(0).X,
                                              new_c.GetEndPoint(0).Y, 0.0)
                                    p1f = XYZ(new_c.GetEndPoint(1).X,
                                              new_c.GetEndPoint(1).Y, 0.0)
                                    pmf = XYZ(pm.X, pm.Y, 0.0)

                                    # Guard: all three points must be distinct
                                    if (p0f.DistanceTo(p1f) > 1e-4 and
                                            p0f.DistanceTo(pmf) > 1e-4 and
                                            p1f.DistanceTo(pmf) > 1e-4):
                                        arc_2d = Arc.Create(p0f, p1f, pmf)
                                        fam_doc.FamilyCreate.NewModelCurve(
                                            arc_2d, sketch_plane)
                                        written = True
                                except Exception:
                                ...
                                if not written:
                                    # Fallback: tessellate arc → line segs
                                    try:
                                ...
                                # NurbSpline / HermiteSpline / Ellipse / etc.
                                # Tessellate → write as projected line segs.
                                written = False
                                try:
                                    pts = new_c.Tessellate()
                                    for i in range(len(pts) - 1):
