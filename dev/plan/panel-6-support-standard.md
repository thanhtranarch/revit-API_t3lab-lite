...
| PDF import | ✅ | Gray-gutter 2026-07-03. **2026-07-05 F11 — 2 vòng sửa**: (v1) grid kẹt "Loading views…" + crash do hoãn nạp sang `ContentRendered`/`Loaded` → nạp đồng bộ trong `__init__`. (v2) mở chậm + crash do DataGrid tắt virtualization → bật lại virtualization + `ViewItem` dùng `INotifyPropertyChanged` + selection qua setter (bỏ event checkbox + `Items.Refresh`). **User xác nhận đã debug hết 2026-07-05** |
...
- **2026-07-05 — PDF import: mở tool không hiện thông tin + crash file (bug thật, user báo — F11)**: Đây là bug functional thật, KHÔNG phải UI — phơi ra rằng xác nhận chung "các tool hoạt động tốt" (2026-07-05) chưa thực sự test functional tool này. Triệu chứng: mở tool thì grid Target Views kẹt ở overlay "Loading views…" (không hiện view nào) và làm crash/đóng file Revit. Truy vết trong `PDFImportDialog.py`:
  ...
  - **Vòng 2 (2026-07-05) — user báo lại: vẫn mở CHẬM + crash file**: root cause còn lại là DataGrid `grid_views` để `EnableRowVirtualization="False"` → model nhiều view thì WPF dựng toàn bộ row cùng lúc (mỗi row có checkbox template + nhiều Path/Border) → chậm + hard-crash. So với `ManaViews` (chạy tốt với nhiều view): nó KHÔNG tắt virtualization và checkbox KHÔNG có event, chỉ bind `IsSelected`. PDFImport phải tắt virtualization vì checkbox có handler `view_selection_changed` gọi `Items.Refresh()` mỗi lần tick (kỵ virtualization). Sửa triệt để: (1) bật lại virtualization (`VirtualizingPanel.IsVirtualizing=True` + `VirtualizationMode=Recycling`, bỏ `EnableRowVirtualization="False"`); (2) `ViewItem` implement `INotifyPropertyChanged` (giống `RenumberItem` của ManaSheets) nên PAGE pill + checkbox tự cập nhật, bỏ mọi `Items.Refresh()`; (3) bỏ event `Checked/Unchecked` XAML, chuyển tính-lại-page + single-select vào **setter `IsSelected`** (`_on_item_toggle`) — setter chỉ chạy khi user thực sự tick (hướng ghi-nguồn của binding), KHÔNG chạy khi row được realize lúc cuộn → an toàn với virtualization. Static clean (`ast.parse` + XML well-formed + 3 audit). **User xác nhận đã debug hết 2026-07-05** — mở nhanh, không crash, chức năng tick/single-select/page-number đúng.
- **2026-07-06 — PDF import: `ImportError: Cannot import name INotifyPropertyChanged` (F11 vòng 3, user báo traceback)**: hệ quả trực tiếp của fix vòng 2 — thêm `from System.ComponentModel import INotifyPropertyChanged, ...` vào `PDFImportDialog.py` nhưng file này **thiếu `clr.AddReference('System')`**. Trên Revit 2026 (pyRevit chạy .NET 8), `INotifyPropertyChanged` nằm trong assembly chỉ được nạp khi add reference `System`; namespace `System.ComponentModel` vẫn resolve được từ các assembly khác đã nạp nên lỗi hiện ra là "Cannot import name" chứ không phải "No module". Tool chỉ chạy được nếu tình cờ một dialog khác (đã AddReference `System`) được import trước trong cùng session. Đối chiếu: TẤT CẢ dialog khác dùng `INotifyPropertyChanged` (ManaSheets, ManaFami, ManaStyles, ManaViews, BatchOut…) đều đã nạp `System` trước import. **Fix**: thêm `clr.AddReference('System')` vào đầu `PDFImportDialog.py`. Chờ user xác nhận mở lại trong Revit.
